...
The **kwpower package** can Perform **Kruskal-Wallis Test** to **Multiple Pairwise-Comparison** between groups to test the mean ranks differ between groups and produce the graphics to visualize the data instead of running Kruskal-Wallis Test, Multiple Pairwise-Comparison separately.
...
The kwpower package comes with two additional functions, **ppt1()** and **ppt2()**, which generate plots for easy comparison. Users can input their dataset, and the plots will be **automatically generated**.

One of the key features of the kwpower package is its ability **to test any type of CSV data** and generate plots with a structure that includes values in the 2nd column and groups in the 3rd column.

**"With just a few clicks, users can perform their analysis easily."**

To read a CSV file, users can choose the file **interactively using a dialog box**. The resulting data can be saved to a variable called my_data.
